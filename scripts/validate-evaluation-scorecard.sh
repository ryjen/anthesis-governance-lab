...
jq -n \
  --argfile catalog "$catalog" \
  --argfile template "$template" \
  --argfile example "$example" '
    ($catalog.collections.canonical.scenarios | map({id, expected_decision})) ==
      ($template.canonical_scenarios | map({id, expected_decision})) and
    ($catalog.collections.canonical.scenarios | map({id, expected_decision})) ==
      ($example.canonical_scenarios | map({id, expected_decision}))
  ' | grep -Fxq true || fail "evaluation scenarios drifted from the canonical catalog"
...
grep -Fq 'bounded reference-composition claim' "$repo_root/docs/evaluation.md" && fail "evaluation guide contains stale wording" || true
